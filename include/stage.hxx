...
namespace BetterSMS::Stage {
    typedef void (*StageInitCallback)(TMarDirector *);
    typedef void (*StageUpdateCallback)(TMarDirector *);
    ...
    bool registerInitCallback(const char *name, StageInitCallback cb);
    bool registerUpdateCallback(const char *name, StageUpdateCallback cb);
...
#define CONSTRUCT_PARAM(name, val)                                                                 \
    name(this, val, JDrama::TNameRef::calcKeyCode(SMS_STRINGIZE(name)), SMS_STRINGIZE(name))
        ...
        TStageParams(const char *prm)
            : TParams(), CONSTRUCT_PARAM(mIsExStage, false), CONSTRUCT_PARAM(mIsDivingStage, false),
              CONSTRUCT_PARAM(mIsOptionStage, false), CONSTRUCT_PARAM(mIsMultiplayerStage, false),
              ...
              CONSTRUCT_PARAM(mPlayerHasGlasses, false), CONSTRUCT_PARAM(mPlayerHasShirt, false),
              ...
              CONSTRUCT_PARAM(mFluddShouldColorWater, false), CONSTRUCT_PARAM(mMusicVolume, 0.75f),
              ...
              CONSTRUCT_PARAM(mMusicEpisodeID, 0), CONSTRUCT_PARAM(mMusicEnabled, true),
              CONSTRUCT_PARAM(mMusicSetCustom, false), CONSTRUCT_PARAM(mGravityMultiplier, 1.0f) {
            delete sStageConfig;
            sStageConfig = this;
...
#undef CONSTRUCT_PARAM
        ...
        static char *stageNameToParamPath(char *dst, const char *stage, bool generalize = false);
