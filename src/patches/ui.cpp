...
static void ensureRedCoinsPanelAdjustOpen(TGCConsole2 *console, int unk_1, s32 time) {
...
static void ensureRedCoinsPanelAdjustClose(TGCConsole2 *console) {
...
static bool ensureJetBalloonAdjustOpen() {
    TGCConsole2 *console;
    SMS_FROM_GPR(30, console);

    if (console->mIsTimerCard) {
        return true;
    }

    // Default behavior (might be better to just return false in this case)
    return console->mJetBalloonCurrent != console->mJetBalloonCoaster;
}
SMS_PATCH_BL(SMS_PORT_REGION(0x8014AC0C, 0, 0, 0), ensureJetBalloonAdjustOpen);
SMS_WRITE_32(SMS_PORT_REGION(0x8014AC14, 0, 0, 0), 0x2C030000);
