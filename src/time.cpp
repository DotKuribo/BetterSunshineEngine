...
BETTER_SMS_FOR_EXPORT void BetterSMS::Time::calendarToDate(char *dst,
                                                           const OSCalendarTime &calendar) {
    snprintf(dst, 32, "%lu/%lu/%lu", calendar.mon + 1, calendar.mday, calendar.year);
}

BETTER_SMS_FOR_EXPORT void BetterSMS::Time::calendarToTime(char *dst,
                                                           const OSCalendarTime &calendar) {
    if (calendar.hour == 0)
